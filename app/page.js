...
export default function Home() {
  ...
  const generateDynamicPrompt = (data) => {
    ...
    if (data.logoFile)
      prompt += `- **Logo:** A logo file has been uploaded to the 'logo' field in the database. Use this file as the primary logo.\n`;
    ...
    prompt += `- **Color Palette:** The primary color is ${
      data.colors[0]
    }. The full palette is: ${data.colors.join(", ")}.\n`;
    ...
    const generateListPrompt = (list, fileFieldName, textDataKey) => {
      ...
      list.forEach((item, index) => {
        let itemDetails = Object.entries(item)
          .filter(
            ([key, value]) =>
              !["fileObject", "id", "previewUrl"].includes(key) && value
          )
        ...
        if (item.fileObject) {
          ...
          itemDetails += `image: "(File ${
            index + 1
          } from the '${fileFieldName}' field)"`;
        }
  ...
  const handleFormSubmit = async (formData) => {
    ...
    const cleanAndStringify = (arr, fieldsToKeep) =>
      JSON.stringify(
        arr.map((item) => {
          let cleanItem = {};
    ...
    fd.append(
      "pages",
      cleanAndStringify(formData.pages, ["slug", "components"])
    );
    fd.append("colors", JSON.stringify(formData.colors));
    fd.append(
      "customFields",
      JSON.stringify(formData.customFields.filter((f) => f.key))
    );
    ...
    processDynamicData("products", "product_images", [
      "name",
      "price",
      "oldPrice",
    ]);
    processDynamicData("portfolioProjects", "portfolio_images", [
      "name",
      "description",
      "projectUrl",
    ]);
    ...
    fd.append(
      "services",
      cleanAndStringify(formData.services, ["name", "description"])
    );
  ...
  return (
    <div className="relative isolate min-h-screen bg-black">
      <div className="absolute inset-x-0 top-[-10rem] -z-10 transform-gpu overflow-hidden blur-3xl sm:top-[-20rem]">
        <div
          className="relative left-1/2 -z-10 aspect-[1155/678] w-[36.125rem] max-w-none -translate-x-1/2 rotate-[30deg] bg-gradient-to-tr from-[#2563eb] to-[#06b6d4] opacity-20 sm:left-[calc(50%-40rem)] sm:w-[72.1875rem]"
          style={{
            clipPath:
      ...
      <main className="container mx-auto px-6 py-12 md:py-24">
        <Header
          lang={lang}
          setLang={setLang}
          content={content[lang] || content.id}
        />
